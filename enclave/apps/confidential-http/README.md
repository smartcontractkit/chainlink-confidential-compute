...
The host program also brokers outbound TCP connections for the enclave. The enclave resolves destinations through the host, validates the returned IPv4 addresses against its request policy, and opens an HTTP CONNECT tunnel over vsock to the exact approved address. TLS remains end to end between the enclave and destination, so the untrusted host relays encrypted bytes without receiving HTTP headers, bodies, credentials, or TLS private state.
