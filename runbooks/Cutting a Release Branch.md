...
### 1. Update the global Confidential Compute version number
...
- In order to run with our E2E tests, our enclave app capability go.mod files use local replaces that point to the current state of our repo. Example: https://github.com/smartcontractkit/confidential-compute/blob/c519f08284afe2f97fa5927f490dd801b8585e4c/enclave/apps/confidential-http/capability/go.mod#L5-L7.
- These replaces must be removed, and the `github.com/smartcontractkit/confidential-compute`, `github.com/smartcontractkit/confidential-compute/capabilities/framework`, and `github.com/smartcontractkit/confidential-compute/enclave-client` imports should point to the most recent commit on `main`. Example: https://github.com/smartcontractkit/confidential-compute/compare/main...release/v0.0.1.
- If an incorrect version of `smartcontractkit/confidential-compute`, `smartcontractkit/confidential-compute/capabilities/framework`, or `smartcontractkit/confidential-compute/enclave-client` is used, the `go-tests` workflow in our CI should fail. Ensure the CI passes before tagging the release branch.
...
Monitor the workflow runs at: https://github.com/smartcontractkit/confidential-compute/actions
...
- EIF file artifacts
Also, copy the link for the summary page of the workflow (example: https://github.com/smartcontractkit/confidential-compute/actions/runs/19507460619) and use this link in step 6 for the release notes.
...
```bash
# Using GitHub CLI
gh release create va.b.c --title "va.b.c" --notes "Enclave measurements for this release can be seen here: [WORKFLOW_SUMMARY_PAGE_LINK_FROM_STEP_5]"
```
...
## Example Release
View the diff for release branch: https://github.com/smartcontractkit/confidential-compute/compare/main...release/v0.0.1
